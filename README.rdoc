...
Note that it is the model responsibility to give the white-list of attributes that can be updated <em>via</em> mass-assignement. In this case, <strong>you must</strong> add the +position+ attribute in the +attr_accessible+ clause.
...
This will trigger some ActiveRecord callbacks in order to maintain positions contiguity across all other concerned items.