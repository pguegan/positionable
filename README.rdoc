...
[![Build Status](https://secure.travis-ci.org/pguegan/positionable.png)](http://travis-ci.org/pguegan/positionable)
...
<b>Caution!</b> The semantic of +next+ or +previous+ methods remain unchanged. More precisely, even if the highest position is the position of the <em>first</em> returned record, it is still considered as the <em>last</em> one (<em>i.e.:</em> +Item.all.first.last?+ returns +true+). I know, this is odd. The semantic of these methods will certainly change in a further version.
