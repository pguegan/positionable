...
It is possible to change this behaviour by setting the <tt>order</tt> option as follows:
...
<strong>Caution!</strong> The semantic of <tt>next</tt> or <tt>previous</tt> methods remain unchanged. More precisely, even if the highest position matches with the <em>first</em> returned record, it is still considered as the <em>last</em> one. This is odd, so the semantic of some methods will certainly be modified in a further version.
